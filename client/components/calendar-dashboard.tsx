...
export function CalendarDashboardSection() {
  ...
  useEffect(() => {
    ...
    const fetchMealDetails = async () => {
      if (!selectedPlan || (!breakfastId && !lunchId && !dinnerId)) {
        if (!cancelled) {
          setMealDetails({
            breakfast: null,
            lunch: null,
  ...
  const summaryDays = useMemo(() => Math.min(planDays, 7), [planDays]);
    ...
    <View className="gap-4 pt-4 pb-6">
      <View>
        ...
        <Text className="text-muted-foreground text-sm mt-1">
          Nutrition, meal plans, and streaks
        </Text>
      </View>
          ...
          <View className="px-4 py-4 border-b border-muted-background flex-row items-center justify-between">
            <View className="flex-row items-center gap-3 flex-1">
              <View className="w-10 h-10 rounded-xl bg-muted-background items-center justify-center">
                <IconSymbol name="fire" size={20} color="--color-foreground" />
              </View>
